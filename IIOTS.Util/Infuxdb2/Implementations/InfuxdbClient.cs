...
namespace IIOTS.Util.Infuxdb2.Implementations
{
    ...
    sealed class InfuxdbClient
    {
        ...
        public async Task<IDataTableCollection> QueryAsync(FluxContent content, string? org)
        {
            ...
            {
                CsvFileHelper csvFileHelper=new CsvFileHelper(stream, Encoding.UTF8);
                var myData = csvFileHelper.GetListCsvData();

              //  var csvReader = new CsvReader(stream, Encoding.UTF8);
              return await DataTableCollection.ParseAsync(myData);
            }
        }
